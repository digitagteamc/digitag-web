...
export default function UserLoginModal({
    ...
    async function verifyOtp(e: React.FormEvent) {
        ...
        try {
            ...
            // backend returns needsRegistration true/false
            const needsRegistration = !!body?.needsRegistration;
            const role = body?.user?.role ?? null;

            onVerified({ phoneNumber: phone, needsRegistration, role });
                    ...
                    <form onSubmit={verifyOtp} className="mt-5 grid gap-3">
                        ...
                        {exists === false && (
                            <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-sm text-white/70">
                                New number detected. After OTP verification you must register as <b>Creator</b> or <b>Brand</b>.
                            </div>
                        )}
